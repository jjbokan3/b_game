...
def play(home_team_id: int, away_team_id: int):
    ...
    current_game = (
        local_session.query(Game)
        .filter_by(
            home_team=home_team_id, away_team=away_team_id, season_id=current_year
        )
        .first()
    )
    ...
    home_batters = (
        local_session.query(Batter)
        .filter((Batter.current_team == home_team_id) & (Batter.num_in_lineup != None))
        .all()
    )
    away_batters = (
        local_session.query(Batter)
        .filter((Batter.current_team == away_team_id) & (Batter.num_in_lineup != None))
        .all()
    )
    home_batters_bench = (
        local_session.query(Batter)
    ...
    home_starters = (
        ...
        .filter((Pitcher.current_team == home_team_id) & (Pitcher.position == "SP"))
        .all()
    )
    away_starters = (
        local_session.query(Pitcher)
        .filter((Pitcher.current_team == away_team_id) & (Pitcher.position == "SP"))
        .all()
    )
    home_relievers = (
        local_session.query(Pitcher)
        .filter((Pitcher.current_team == home_team_id) & (Pitcher.position == "RP"))
        .all()
    )
    away_relievers = (
        local_session.query(Pitcher)
        .filter((Pitcher.current_team == away_team_id) & (Pitcher.position == "RP"))
        .all()
    )
    ...
    home_batters_stats: list[BatterGameStats] = [
        BatterGameStats(batter.id, current_game.id) for batter in home_batters
    ]
    away_batters_stats: list[BatterGameStats] = [
        BatterGameStats(batter.id, current_game.id) for batter in away_batters
    ]
    home_starters_stats: list[PitcherGameStats] = [
        PitcherGameStats(pitcher.id, current_game.id) for pitcher in home_starters
    ]
    away_starters_stats: list[PitcherGameStats] = [
        PitcherGameStats(pitcher.id, current_game.id) for pitcher in away_starters
    ]
    home_relievers_stats: list[PitcherGameStats] = [
        PitcherGameStats(pitcher.id, current_game.id) for pitcher in home_relievers
    ]
    away_relievers_stats: list[PitcherGameStats] = [
        PitcherGameStats(pitcher.id, current_game.id) for pitcher in away_relievers
    ]
    ...
    for x in [
        ...
        home_starters_stats,
        away_starters_stats,
        home_relievers_stats,
        away_relievers_stats,
    ]:
        for y in x:
            local_session.add(y)
    ...
    while total_outs != max_outs:
        while inning_outs < 3:
            if home_hitting:
                outcome, pitch_count_inc = play_ball(
                    away_pitcher, home_batters[home_batter % 9]
                )
                if away_reliever_number == 0:
                    current_pitcher_stats = [
                        pitcher_stats
                        for pitcher_stats in away_starters_stats
                        if pitcher_stats.pitcher_id == away_pitcher.id
                    ][0]
                else:
                    current_pitcher_stats = [
                        pitcher_stats
                        for pitcher_stats in away_relievers_stats
                        if pitcher_stats.pitcher_id == away_pitcher.id
                    ][0]
                (
                    bases,
                    ...
                    inning_outs,
                    away_pitcher,
                    home_batters[home_batter % 9],
                    current_pitcher_stats,
                    home_batters_stats,
                )
                home_batter += 1
                if (
                    away_pitcher.position == "SP"
                    and away_pitch_count > starter_pitch_limit
                ):
                    ...
                    away_pitcher.position == "RP"
                    and away_pitch_count > reliever_pitch_limit
                ):
                ...
                outcome, pitch_count_inc = play_ball(
                    home_pitcher, away_batters[away_batter % 9]
                )
                if home_reliever_number == 0:
                    current_pitcher_stats = [
                        pitcher_stats
                        for pitcher_stats in home_starters_stats
                        if pitcher_stats.pitcher_id == home_pitcher.id
                    ][0]
                else:
                    current_pitcher_stats = [
                        pitcher_stats
                        for pitcher_stats in home_relievers_stats
                        if pitcher_stats.pitcher_id == home_pitcher.id
                    ][0]
                (
                    bases,
                    ...
                    inning_outs,
                    home_pitcher,
                    away_batters[away_batter % 9],
                    current_pitcher_stats,
                    away_batters_stats,
                )
                away_batter += 1
                if (
                    home_pitcher.position == "SP"
                    and home_pitch_count > starter_pitch_limit
                ):
                    home_pitcher = home_relievers[home_reliever_number]
                    home_reliever_number += 1
                    home_pitch_count = 0
                elif (
                    home_pitcher.position == "RP"
                    and home_pitch_count > reliever_pitch_limit
                ):
                    home_pitcher = home_relievers[home_reliever_number]
                    home_reliever_number += 1
                    home_pitch_count = 0
    ...
    no_pitching = [
        [hs for hs in home_starters_stats if hs.pitches == 0],
        [aws for aws in away_starters_stats if aws.pitches == 0],
        [hr for hr in home_relievers_stats if hr.pitches == 0],
        [awr for awr in away_relievers_stats if awr.pitches == 0],
    ]
...
def outcome_modifier(
    ...
    batters_stats: list[BatterGameStats],
):

    # TODO: Can you just pass in a game object and modify that instead of passing in all these variables?
    ...
    current_batter_stats = [
        batter_stats
        for batter_stats in batters_stats
        if batter_stats.batter_id == batter.id
    ][
        0
    ]  # Grab current batter stats

    current_batter_stats.rbis += len(scorers)  # Increment RBIs based on how many scored

    if (
        batter.id in scorers
    ):  # If the current batter is also in scorers (home run) increment runs by 1
        current_batter_stats.runs += 1

    if (
        outcome != "walk"
    ):  # Increment at bats only if the outcome is not a walk TODO: Strikeouts, sac fly, HBP
        current_batter_stats.at_bats += 1

    outcome_plural = (
        outcome + "s"
    )  # Make outcome plural (single is an outcome; singles is an attribute of stat object)

    if outcome_plural != "outs":  # Add outcome to stat object
        setattr(
            current_batter_stats,
            outcome_plural,
            getattr(current_batter_stats, outcome_plural) + 1,
        )
    ...
    if len(scorers) != 0:
        scorer_stats = [
            runner_stats
            for runner_stats in batters_stats
            if runner_stats.batter_id in scorers
        ]
    ...
    if outcome_plural != "outs":
        setattr(
            pitcher_stats, outcome_plural, getattr(pitcher_stats, outcome_plural) + 1
        )
...
def play_ball(pitcher: Pitcher, batter: Batter):
    ...
    triple = ma["3B"] * (
        (batter.attributes["speed/stealing"] / 80 + batter.attributes["power"] / 90) / 2
    )
    ...
    # TODO: Add a distribution for pitches

    outcome = random.choices(
        ["single", "double", "triple", "home_run", "walk", "out"],
        [single, double, triple, home_run, walk, out],
    )
