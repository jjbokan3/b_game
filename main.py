...
def assign_parent_position():
    return random.choices(
        list(PARENT_POSITION_PROB.keys()), list(PARENT_POSITION_PROB.values())
    )[0]


def assign_position(position: str):
    return random.choices(
        list(POSITION_PROB[position].keys()), list(POSITION_PROB[position].values())
    )[0]
...
def assign_handed(position: str) -> str:
    ...
    """
    return random.choices(
        list(HANDED_PROB[position].keys()), list(HANDED_PROB[position].values())
    )[0]
...
def create_player_df() -> pd.DataFrame:
    df_main = pd.DataFrame(
        np.around(
            np.random.randint(
                LOWER_BOUND_MAIN_RATING, HIGHER_BOUND_MAIN_RATING, NUM_PLAYERS
            )
        ),
        columns=["main_rating"],
    )
    df_main["parent_position"] = df_main.apply(
        lambda row: assign_parent_position(), axis=1
    )
    df_main["position"] = df_main.apply(
        lambda row: assign_position(row["parent_position"]), axis=1
    )
    df_main["handed"] = df_main.apply(
        lambda row: assign_handed(row["parent_position"]), axis=1
    )
    df_main["attributes"] = df_main.apply(
        lambda row: (
            assign_hitting_attributes(row["main_rating"])
            if row["parent_position"] == "B"
            else assign_pitching_attributes(row["main_rating"])
        ),
        axis=1,
    )
...
class Pitcher(Player):
    """
    Pitcher object that inherits Player, while adding position specific stats
    """
...
class Game(Base):
    ...
    def __init__(
        self, home_team, away_team, season_id, home_score=0, away_score=0, date=None
    ):
...
class PitcherGameStats(Base):
    ...
    def calc_whip(self):
        try:
            return (
                self.singles + self.doubles + self.triples + self.home_runs + self.walks
    ...
    def __add__(self, other):
        if self.batter_id != other.batter_id:
            ...
            #       Just display it as decimal but keep as total outs?
...
class BatterGameStats(Base):
    ...
    def __add__(self, other):
        if self.batter_id != other.batter_id:
            ...
            result = BatterGameStats(
                self.batter_id,
                self.game_id,
                self.at_bats + other.at_bats,
                self.runs + other.runs,
                self.rbis + other.rbis,
                self.singles + other.singles,
                self.doubles + other.doubles,
                self.triples + other.triples,
                self.home_runs + other.home_runs,
